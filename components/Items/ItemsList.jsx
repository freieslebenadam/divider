  ...
  return (
    <div className="flex flex-col gap-2 my-1">
      {items.map(item => {
        ...
        return (
          <ListItem key={item.id} item={{...item, price: formatPrice(item.price)}} />
        )
      })}
      <div className="text-sm flex">
        <div className="font-normal px-4 py-3 flex-auto"><span className="font-bold">{items.length}</span> {items.length === 0 ? "Položek": items.length === 1 ? "Položka" : "Položky"}</div>
        <div className="flex-none font-mono py-3 px-4 font-semibold text-neutral-500">
          ...
          <span>{formatPrice(sumTotal())}</span>
